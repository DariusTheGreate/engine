...
std::vector<CustomWindow> UI::customWindows;
...
void UI::pushCustomWindow(const CustomWindow& wnd)
{
	...
	customWindows.push_back(wnd);
}

void UI::updateCustomWindows()
{
	for (auto& wnd : customWindows)
	{
		if (!wnd.isOpen) 
		{
			continue;
		...
		ImGui::Begin(wnd.name.c_str(), &wnd.isOpen);
